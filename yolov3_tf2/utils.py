...
def draw_outputs(img, outputs, class_names): # This is used for detection on images and objects
    colors = ((np.array(color_palette("hls", 80)) * 255)).astype(np.uint8) # 80 colors, each class has its own color
    ...
    for i in range(nums):
        ...
        x0, y0 = x1y1[0], x1y1[1] # Save the top left corner for later, when the text will be written
...
def freeze_all(model, frozen=True): # Used to freeze the layers of the model
    model.trainable = not frozen # or unfreeze them for training
